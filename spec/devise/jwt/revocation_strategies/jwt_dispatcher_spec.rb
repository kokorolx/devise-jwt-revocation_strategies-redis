...
RSpec.describe Devise::Jwt::RevocationStrategies::Redis::JwtDispatcher do
  ...
  describe '#on_jwt_dispatch' do
    ...
    it 'saves the JWT in Redis' do
      expect($redis_auth).to receive(:sadd).with("jwt:#{payload['sub']}", payload['jti'])
      expect($redis_auth).to receive(:expireat).with("jwt:#{payload['sub']}", payload['exp'])
    ...
    context 'when payload is nil' do
      it 'does not attempt to save to Redis' do
        expect($redis_auth).not_to receive(:sadd)
  ...
  describe '#save_token_in_redis' do
    ...
    it 'sadds the token in Redis with the correct key and value' do
      expect($redis_auth).to receive(:sadd).with("jwt:#{user_id}", jti)
      expect($redis_auth).to receive(:expireat).with("jwt:#{user_id}", exp)
